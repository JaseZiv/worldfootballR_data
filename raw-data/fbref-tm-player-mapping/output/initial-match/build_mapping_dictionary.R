...
playing_time <- fb_big5_advanced_season_stats(season_end_year = c(2018:2022),
                                              stat_type = "playing_time",
                                              team_or_player = "player")
saveRDS(playing_time, "fbref_players.rds")


valuations <- get_player_market_values(country_name = c("England", "Spain", "France", "Italy", "Germany"),
                                       start_year = c(2017:2021))
saveRDS(valuations, "tm_players.rds")

###############################################################################################################

fbref <- readRDS("fbref_players.rds")
tm <- readRDS("tm_players.rds")
...
write.csv(joined_finished, here("output", "initial-match", "joined_finished.csv"), row.names = F)
...
write.csv(duplicate_players, here("output", "initial-match", "working-files", "duplicate_players_df.csv"), row.names = F)
